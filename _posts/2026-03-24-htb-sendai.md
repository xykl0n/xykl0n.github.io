...
Sendai is a Medium-rated Windows Active Directory machine from HackTheBox. The entry point is deceptively simple — guest SMB access reveals a company share with an incident report announcing that accounts with weak passwords have been expired. That's not a warning, that's an invitation. RID cycling surfaces expired accounts, a null password spray confirms them, and `nxc change-password` resets them without knowing the current credentials. From there the box splits into two paths: the intended route chains ACL abuse → GMSA password read → service binary credential leak → ADCS ESC4 template overwrite to land Administrator. The unintended route finds SQL credentials in a config share, tunnels through Ligolo-ng to reach a localhost MSSQL instance, forges a Silver Ticket to connect as Administrator, and abuses `SeImpersonatePrivilege` via GodPotato for SYSTEM.
...
![[sendai-bloodhound-1.png]]

![[sendai-bloodhound-2.png]]
...
![[sendai-bloodhound-caoperators.png]]
...
![[sendai-template-overwrite.png]]

Modern AD validates both the UPN and the Object SID embedded in the certificate. Without the correct SID, `certipy auth` returns "Object SID mismatch". Recover the Administrator SID first:

![[sendai-sid-lookup.png]]
...
|-----------|------------|
| Guest SMB Enumeration | sendai share → incident.txt → expired account disclosure |
| RID Cycling | IPC$ guest access → user enumeration |
| STATUS_PASSWORD_MUST_CHANGE Abuse | nxc change-password without knowing current password |
| GenericAll ACL Abuse | support → ADMSVC group membership add |
| GMSA Password Read | ADMSVC → ReadGMSAPassword → mgtsvc$ NT hash |
| Service Binary Credential Leak | PrivescCheck → helpdesk.exe ImagePath → clifford.davey |
| ADCS ESC4 | certipy template overwrite → ESC1-equivalent → Administrator hash |
| Object SID Embedding | lookupsid.py → certipy req -sid to bypass SID validation |
| Silver Ticket Forgery | sqlsvc NT hash → Administrator TGS for MSSQL SPN |
| Ligolo-ng Tunneling | localhost MSSQL access via agent pivot |
| SeImpersonatePrivilege Abuse | GodPotato → SYSTEM via MSSQL xp_cmdshell |
...
- [Silver Ticket - HackTricks](https://book.hacktricks.xyz/windows-hardening/active-directory-methodology/silver-ticket)
